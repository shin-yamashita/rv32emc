...
for cols in trace:
  if('cnt' in cols[0]):
    ...
    cnt,pc,ir,opc,opr,mar,mdr,mdw,rrd1,rrd2,alu,rwa,rwd,rwdat = cols[0:14]
#   if(int(cols[1],16)>0):
...
#    refs.append("%8x %8x %8x %8x %8x %8x"%(int(cols[1],16),int(cols[2],16),int(cols[5],16),                  int(cols[8],16),int(cols[9],16),int(cols[13],16)))
    refs.append("%8x %8x %8x %8x %8x"%(int(pc,16), int(ir,16), int(rrd1,16), int(rrd2,16), int(rwdat,16)))
