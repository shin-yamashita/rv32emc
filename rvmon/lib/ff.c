...
#else
void memcpydma(void* dst, const void* src, UINT len);
#define mem_cpy		memcpydma
#endif
...
#else	/* Non LFN configuration */
    res = dir_sdi(dj, 0);
    //d_printf("dir_register:dir_sdi:%d\n",res);
    if (res == FR_OK) {
...
)
{
    ...
    dir = dj.dir;
    d_printf("=%d %x dir:%x '%s'\n", __LINE__, res,dir,dir);
