...
static inline void spi_br(int br)
{
	*SPIBR = br;
}
static inline void spi_cs(int cs)
{
        *SPISCS = cs;
}
static inline void spi_tx(u8 data)
{
        while(*SPIFLG & 0x2);   // txfull
        *SPITRX = data;
}
//---
static inline void spi_tx_blk(u8 data)
{
//        while(*SPIFLG & 0x2);   // txfull
        *SPITRXBLK = data;
}
static inline u8 spi_rx()
{
        u8 data;
...
static inline u8 spi_trx_blk()  // ok
{
//      while(*SPIFLG & 0x2);   // txfull
        *SPITRXBLK = 0xff;
//      while(*SPIFLG & 0x2);   // txfull (wait tx complete)
...
d_printf("CMD8 buf:%2x %2x %2x %2x\n", buf[0],buf[1],buf[2],buf[3]);
			if (buf[2] == 0x01 && buf[3] == 0xAA) {		/* The card can work at vdd range of 2.7-3.6V */
				for (tmr = 1000; tmr; tmr--) {		/* Wait for leaving idle state (ACMD41 with HCS bit) */
					if (send_cmd(ACMD41, 1UL << 30) == 0) break;
					DLY_US(1000);
				}
d_printf("ACMD41 tmr:%d\n", tmr);
				if (tmr && send_cmd(CMD58, 0) == 0) {	/* Check CCS bit in the OCR */
					rcvr_mmc(buf, 4);
					ty = (buf[0] & 0x40) ? CT_SD2 | CT_BLOCK : CT_SD2;	/* SDv2 */
d_printf("CMD58 buf:%2x\n", buf[0]);
				}
			}
d_printf("CMD8 ty:%d\n", ty);
		} else {					/* SDv1 or MMCv3 */
